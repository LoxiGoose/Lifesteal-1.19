...
public class CapabilityStorage<S extends INBT, C extends INBTSerializable<S>> implements Capability.IStorage<C> {
    ...
    protected CapabilityStorage(final int expectedTagType, @Nonnull final Lazy<Capability<C>> capability) {
        this.expectedTagType = expectedTagType;
        this.capability = capability;
    }

    @Nonnull
    public static <S extends INBT, C extends INBTSerializable<S>> CapabilityStorage<S, C> create(@Nonnull final Supplier<Capability<C>> capability) {
        return create(capability, -1);
    }

    @Nonnull
    public static <S extends INBT, C extends INBTSerializable<S>> CapabilityStorage<S, C> create(@Nonnull final Supplier<Capability<C>> capability, final int expectedTagType) {
        return new CapabilityStorage<>(expectedTagType, Lazy.of(capability));
    }
